...
async function publicLookup(){
  ...
  box.innerHTML=data.map(x=>`<article class="lookup-item"><div><b>${x.product||'Không rõ máy'}</b><span>${x.customer||''} · ${x.phone||''}</span></div><p>${x.repairId||''} · IMEI ${x.imei||''}</p><section><em class="state ${tone(x.status)}">${cleanStatus(x.status)}</em><strong>${money(x.serviceTotal||x.estimate)}</strong></section><small>Hẹn trả: ${parsePublicDate(x.appointment)||'Chưa hẹn'}</small><button class="ghost-btn" onclick='openPublicReceipt(${JSON.stringify(x).replace(/'/g,"&#39;")},false)'>In phiếu nhận</button></article>`).join('')
}
