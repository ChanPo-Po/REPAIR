...
function renderTechQueue(data){
  ...
  box.innerHTML=active.slice(0,40).map(x=>`<button class="job-card" onclick="prefillStatus('${x.repairId}')"><div><b>${x.product||'Không rõ máy'}</b><span>${x.customer||''} · ${x.phone||''}</span></div><em class="status-dot ${statusTone(x.status)}">${cleanStatus(x.status)}</em><small>${x.repairId} · Hẹn ${fmtDate(x.appointment)||'—'}</small></button>`).join('')||'<p class="empty-note">Không có máy cần xử lý.</p>'
}
