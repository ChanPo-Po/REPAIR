...
function renderModels(data){
  ...
  const serviceModel=Object.entries(byService).map(([service,arr])=>{
    const models=topFromGroups(groupBy(arr,x=>x.model),a=>a.length);
    return {service,total:arr.length,models};
  }).sort((a,b)=>b.total-a.total).slice(0,10);

  const box=document.getElementById('modelAnalysis');
  if(box){
    box.innerHTML=serviceModel.map(sv=>`<div class="service-model-card"><div><b>${sv.service}</b><span>${sv.total} lượt dịch vụ</span></div><div class="model-chips">${sv.models.slice(0,6).map(m=>`<em>${m.name}: ${m.value}</em>`).join('')}</div></div>`).join('')||'<p class="muted">Chưa có dữ liệu</p>';
  }

  // Ma trận đúng logic: DÒNG MÁY x DỊCH VỤ.
  // Mỗi ô = số lần dịch vụ đó được cập nhật cho dòng máy đó.
  const matrix=document.getElementById('commonIssues');
  if(!matrix)return;
  const services=Object.keys(byService).sort((a,b)=>byService[b].length-byService[a].length).slice(0,12);
  const models=[...new Set(facts.map(x=>x.model).filter(Boolean))].sort();
  if(!services.length || !models.length){matrix.innerHTML='<p class="muted">Chưa có dữ liệu</p>';return;}
  const countMap={};
  facts.forEach(x=>{
    const key=x.model+'||'+x.service;
    countMap[key]=(countMap[key]||0)+1;
  });
  matrix.innerHTML=`
    <div class="matrix-table-wrap">
      <table class="service-matrix-table">
        <thead>
        ...
        <tbody>
          ${models.map(model=>`<tr><th>${safe(model)}</th>${services.map(service=>`<td>${countMap[model+'||'+service]||''}</td>`).join('')}</tr>`).join('')}
        </tbody>
      </table>
    </div>`;
}
...
async function loadRepairList(){await getRows();const q=(document.getElementById('listKeyword')?.value||'').toLowerCase();const st=document.getElementById('listStatus')?.value||'';const tech=document.getElementById('listTech')?.value||'';const data=filteredRows().filter(x=>(!q||JSON.stringify(x).toLowerCase().includes(q))&&(!st||x.status===st)&&(!tech||x.technician===tech));const tbody=document.getElementById('repairRows');if(tbody)tbody.innerHTML=data.map(x=>`<tr><td><b>${x.repairId||''}</b></td><td>${fmtDate(x.date)}</td><td>${x.branch||''}</td><td>${x.product||''}</td><td>${x.customer||''}</td><td>${x.phone||''}</td><td><span class="status-dot ${statusTone(x.status)}">${cleanStatus(x.status)}</span></td><td>${x.technician||'—'}</td><td>${fmtDate(x.appointment)}</td><td><b>${money(rev(x))}</b></td><td><div class="row-actions"><button class="view-btn" onclick="openDetail('${x.repairId}')">Chi tiết</button><button class="view-btn light" onclick="openTab('status');prefillStatus('${x.repairId}')">Cập nhật</button>${String(x.status||'').startsWith('7.')||String(x.status||'').startsWith('8.')?'':`<button class="view-btn success" onclick="quickDone('${x.repairId}')">Đã sửa xong</button>`}${String(x.status||'').startsWith('8.')?'':`<button class="view-btn success" onclick="quickReturn('${x.repairId}')">Đã trả</button>`}</div></td></tr>`).join('')||'<tr><td colspan="11">Không có dữ liệu</td></tr>';renderTechQueue(data)}
function renderTechQueue(data){
  const box=document.getElementById('techQueue'); if(!box)return;
  const active=data.filter(x=>!isClosedStatus(x.status));
  box.innerHTML=active.slice(0,40).map(x=>`<div class="job-card"><button class="job-main" onclick="prefillStatus('${x.repairId}')"><div><b>${x.product||'Không rõ máy'}</b><span>${x.customer||''} · ${x.phone||''}</span></div><em class="status-dot ${statusTone(x.status)}">${cleanStatus(x.status)}</em><small>${x.repairId} · Hẹn ${fmtDate(x.appointment)||'—'}</small></button><div class="job-actions"><button class="view-btn light" onclick="prefillStatus('${x.repairId}')">Cập nhật</button>${String(x.status||'').startsWith('7.')||String(x.status||'').startsWith('8.')?'':`<button class="view-btn success" onclick="quickDone('${x.repairId}')">Đã sửa xong</button>`}</div></div>`).join('')||'<p class="empty-note">Không có máy cần xử lý.</p>'
}
...
document.getElementById('statusForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target),d=Object.fromEntries(f.entries());const res=await api('updateStatus',{repairId:d.repairId,data:{actualStatus:d.actualStatus,repairService:d.repairService,estimate:d.estimate,place:d.processPlace,technician:d.technician,status:d.status,techNote:d.techNote,updatedAt:new Date().toLocaleString('vi-VN'),user:currentUser?.name||currentUser?.role||''}});if(!res.success)return notifyError(res.message||'Không cập nhật được trạng thái.');await getRows();loadRepairList();loadDashboard();document.getElementById('statusForm')?.classList.add('hidden');notifySuccess('Đã cập nhật trạng thái phiếu '+d.repairId,'Cập nhật thành công');if(String(d.status||'').startsWith('8.')){notifyPopup('Đã trả khách','Bạn có muốn in phiếu trả cho khách ký không?','success',[{label:'In phiếu trả',className:'primary-btn',onClick:()=>printReceipt('return',d.repairId)},{label:'Bỏ qua',className:'ghost-btn',onClick:()=>{}}]);}if(currentUser?.role==='kythuat'){openTab('status');window.scrollTo({top:0,behavior:'smooth'});}else{openTab('list');}});
async function findForCost(){const q=document.getElementById('costKeyword').value.trim();const res=await api('search',{q});const x=res.data?.[0];if(!x)return notifyWarn('Không tìm thấy phiếu theo từ khóa này.','Không thấy phiếu');const f=document.getElementById('costForm');f.classList.remove('hidden');f.repairId.value=x.repairId;if(f.materialBill)f.materialBill.value=x.materialBill||'';if(f.materialName)f.materialName.value=x.materialName||'';f.materialCost.value=x.materialCost||0;f.laborCost.value=x.laborCost||0;f.actualRevenue.value=x.actualRevenue||x.estimate||0;if(f.supplier)f.supplier.value=x.supplier||'';f.paymentStatus.value=x.paymentStatus||'Chưa thanh toán';}
document.getElementById('costForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target),d=Object.fromEntries(f.entries());const res=await api('updateCost',{repairId:d.repairId,data:d});if(!res.success)return notifyError(res.message||'Không cập nhật được chi phí.');notifySuccess('Đã lưu chi phí / thực thu cho phiếu '+d.repairId,'Cập nhật chi phí xong');loadRepairList();loadDashboard();});
...
function renderMaterials(){const box=document.getElementById('materialsList');if(!box)return;box.innerHTML=DM_VAT_TU.map(x=>`<div class="card"><h4>${x.name}</h4><p>${x.group||''}</p></div>`).join('')}
